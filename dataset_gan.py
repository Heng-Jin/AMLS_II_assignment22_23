...
class amls_dataset(Dataset):
    ...
    def __init__(self, path, mode):
        ...
        for key, x_path in x_paths_dict.items():
            ...
            # if ind_x != ind_y:
            #     logging.error('-----LR / HR index Mismatch-----')
            #     print('-----LR: %s / HR: %s index Mismatch-----', x_path.stem, y_path.stem)
    ...
    def __getitem__(self, index):
        ...
        #     cb_lr = transforms_lr(cb_lr)
        #     cr_lr = transforms_lr(cr_lr)
        #     return img_lr, torch.stack((cb_lr, cr_lr), dim=0)
